...
One command, three modes: `/turtleneck init | review | pass`.
...
system", "design review / pass", "set up design knowledge", or `/turtleneck …`.
...
4. **Respect the read-only / mutate split.** `review` reports only and never
   edits. `pass` may edit, but only behind a human-approved diff. Ambiguous or
   empty input must never land in `pass` — show the menu. Never silently rewrite.
...
| `/turtleneck pass` | Review + apply improvements behind an approved diff. |
